...
/**
 * Code-risk must mean "evaluate concrete code", not merely "a programming-language concept".
 * This intentionally excludes questions such as "C 프로그램의 필수 함수는?", operator
 * precedence, language identification, or keyword-selection unless the question asks for
 * an actual execution/output/value from a concrete snippet.
 */
function isCodeExecution(q) {
  const qt = questionText(q);
  const hasSnippet = /`[^`]{3,}`/.test(qt);
  const explicitProgram = /(?:다음\s+)?(?:C|C언어|JAVA|Java|Python|파이썬)\s*(?:언어\s*)?(?:프로그램|코드)[\s\S]*(?:실행|출력|결과)/i.test(qt)
    || /(?:프로그램|코드)[\s\S]*(?:실행되었|실행했|실행\s*결과|출력\s*결과)/i.test(qt);
  const resultCue = /실행\s*(?:결과|하면|후)|실행되었을\s*때|출력(?:되는|은|값|결과|하면|형태)|결과(?:는|값|로)|값(?:은|을)|구하면|계산하면|후\s+[A-Za-z_가-힣]\w*\s*(?:는|값)/i.test(qt);
  const concreteSyntaxInQuestion = /#include|printf\s*\(|scanf\s*\(|System\.out|public\s+static\s+void|\bdef\s+[A-Za-z_]\w*\s*\(|print\s*\(|\bfor\s*\([^)]*;[^)]*;[^)]*\)|\bwhile\s*\([^)]*\)|\bswitch\s*\(|\bsizeof\s*\(|\bint\s+[A-Za-z_]\w*\s*=|\bchar\s+[A-Za-z_]\w*\s*=|\+\+|--/i.test(qt);
  return explicitProgram || (hasSnippet && resultCue) || (concreteSyntaxInQuestion && resultCue);
}

function isSqlExecution(q) {
  const qt = questionText(q);
  return /\bSELECT\b[\s\S]*\bFROM\b/i.test(qt)
    || /SQL\s*(?:문|구문|질의|실행|결과)|서브쿼리|CREATE\s+VIEW|DROP\s+VIEW|ALTER\s+TABLE|\bUNION(?:\s+ALL)?\b|\bINTERSECT\b|\bMINUS\b/i.test(qt);
}

function isNetworkCalculation(q) {
  const qt = questionText(q);
  return /(?:\d{1,3}\.){3}\d{1,3}\/\d{1,2}/.test(qt)
    || /CIDR|서브넷\s*마스크|서브넷으로|브로드캐스트\s*주소|Broadcast\s*주소|사용\s*가능(?:한)?\s*IP|Subnet-Zero|FLSM|VLSM|네트워크\s*주소를\s*구/i.test(qt);
}

/**
 * Numerical/trace algorithm risk only. Concept-only mentions of COCOMO, PERT, LRU, etc.
 * are not treated as calculation risk.
 */
function isAlgorithmCalculation(q) {
  const qt = questionText(q);

  const schedulingTrace =
    /(?:FCFS|FIFO\s*스케줄링|\bSJF\b|\bSRT\b|\bHRN\b|Round\s*Robin|라운드\s*로빈)/i.test(qt)
    && /(?:\d|평균\s*(?:대기|반환)시간|반환시간|Turnaround|Waiting\s*Time|작업\s*순서|처리\s*순서|가장\s*먼저)/i.test(qt);

  const pageTrace =
    /(?:페이지\s*교체|\bLRU\b|\bLFU\b|\bNUR\b|FIFO)/i.test(qt)
    && /(?:프레임|참조\s*(?:순서|열)|페이지\s*(?:결함|부재)|횟수|마지막에\s*남는)/i.test(qt);

  const memoryTrace =
    /(?:Best\s*Fit|Worst\s*Fit|First\s*Fit|메모리\s*할당)/i.test(qt)
    && /(?:KB|K\b|공간|단편화|프로그램|No\.\d|\d)/i.test(qt);

  const diskTrace =
    /(?:SSTF|SCAN|C-SCAN|LOOK|디스크\s*스케줄링)/i.test(qt)
    && /(?:헤드|요청\s*큐|처리\s*순서|탐색|\d)/i.test(qt);

  const projectCalc =
    /(?:LoC|라인|Man-Month|COCOMO|기능\s*점수|Function\s*Point|\bCPM\b|\bPERT\b|임계\s*경로|생산성)/i.test(qt)
    && /(?:\d|계산|구하면|소요\s*(?:기간|기일)|경로.*일|월간\s*생산성|Man-Month)/i.test(qt);

  return schedulingTrace || pageTrace || memoryTrace || diskTrace || projectCalc;
}
...
function riskFor(q) {
  ...
  const visualDependency =
    /다음\s*(?:그림|표|도표)|제시된\s*(?:R|S|테이블|릴레이션|그림|표)|①의\s*결과|②의\s*결과|③의\s*결과|④의\s*결과|두\s*릴레이션.*결과|테이블.*실행\s*결과/i.test(qt);
  if (visualDependency) {
    score += 3;
...
function queueItem(q, risk) {
  return {
    ...
    text: questionText(q),
    reviewOutcome: q.reviewOutcome || null,
    ...risk
  };
}
...
const sorter = (a, b) =>
  ...
  || String(a.page).localeCompare(String(b.page), undefined, { numeric: true });

queue.sort(sorter);
independentlyVerified.sort(sorter);
reviewedWarnings.sort(sorter);
...
for (const subject of ['1과목', '2과목', '3과목', '4과목', '5과목']) {
  console.log(`${subject}: ${bySubject[subject] || 0}`);
}
...
for (const [key, label] of [
  ['code', '코드 실행'],
  ['sql', 'SQL 실행'],
  ['network_calc', '네트워크 계산'],
  ['algorithm_calc', '계산/알고리즘'],
  ['crypto', '암호/보안']
]) {
...
if (!summaryOnly) {
  ...
  if (reviewedWarnings.length) {
    ...
    for (const item of reviewedWarnings.slice(0, 20)) {
      console.log(`- ${item.id} ${item.reviewOutcome || 'reviewed_warning'}`);
    }
  }
}
